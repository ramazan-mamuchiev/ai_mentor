...
const MOBILE_BP = 768
const RATIO_KEY = 'ipcodex-debug-panel-ratio'
...
export function ChatDebugPanel({ debug, sessionId, messageId, onClose }: Props) {
  ...
  const heightPercent = `${(ratio * 100).toFixed(2)}%`
  const panelStyle = isMobile ? undefined : { height: heightPercent, minHeight: heightPercent }
  ...
  return (
    <>
      ...
      {!isMobile && (
        <div
          className="debug-panel-splitter"
      ...
      <div ref={containerRef} className="chat-debug-panel" style={panelStyle}>
        <div className="debug-panel-header">
          <div className="debug-panel-header-content">
            <span className="debug-panel-title">
              <Bug size={14} />
              {t('chat.debugPanel.title')}
            </span>
            {(sessionId != null || messageId != null) && (
              <span className="debug-panel-ids">
